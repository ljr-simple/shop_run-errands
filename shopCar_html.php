...
<html>
	...
	<nav class="navbar navbar-default">
		...
		<div id="mid">
			...
			<?php if(empty($shopCarProductArr)){?>
				<div class="oneGood" >
					<h3>待接单<h3>
				</div>
			<?php } else { ?>
		...
		<hr>
			...
			<?php } else { ?>
				<div class="good_total">
			<?php foreach($tpro as $key =>$v): ?>
			<div class="good">			
				<input type="hidden" name="hid" id="hid" value="<?php echo $key;?>" />
				<input type="hidden" name="hidPid" id="hidPid" value="<?php echo $tpro[$key]["Pid"];?>" />
				<div class="oneGood" >
					<img  src="yuansheng_houtai/Public/Uploads/<?php echo $tpro[$key]["PIMG"] ?>" width="300px" height="300px"/>
					
					<div class="GoodName">
						发 布 人：&nbsp&nbsp
					<?php echo $tpro[$key]["Puser"] ?><br>
					任务详情:&nbsp&nbsp&nbsp&nbsp
					<?php echo $tpro[$key]["Pname"] ?><br>
					配送地址:&nbsp&nbsp&nbsp&nbsp
					<?php echo $tpro[$key]["Pword"] ?>
					 
					<!--该商品id-->
					<input type="hidden" name="pid[]" id="" value="<?php echo $tpro[$key]["Pid"];?>" />
			
					<div class="oneMoney">
						单份价格：&nbsp&nbsp&nbsp<span class="aMoney"><?php echo $tpro[$key]["Ptotal"] ?></span>
					</div>
					<div class="numAndHow">
						任务数量：&nbsp&nbsp <span class="shopNum" name="shopNum[]" id="shopNum" value="" >1</span>
						<!-- <input class="shopNum" type="text" name="shopNum[]" id="shopNum" value="1" /> -->
